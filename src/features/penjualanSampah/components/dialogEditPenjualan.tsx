...
import { editPenjualan } from "../mutations"

type DialogEditPenjualanProps = {
    data: Penjualan
    refetch: () => void
}

function DialogEditPenjualan({ data, refetch }: DialogEditPenjualanProps) {
    const {
        register,
        handleSubmit,
        reset
    ...
    const onSubmit = handleSubmit(async (data) => {
        try {
            ...
            refetch()
            toast.success("Berhasil mengubah data penjualan")
        }
        catch {
            toast.error("Gagal mengubah data penjualan")
        }
        reset()
    })
        ...
        <Dialog
            trigger={
                <button className="flex">
                    <NotePencil className="mr-2" size={24} /> Edit Data Penjualan
                </button>
            }
...
export default DialogEditPenjualan